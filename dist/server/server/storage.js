...
export class DatabaseStorage {
    ...
    async updateDailyProgressForDate(date) {
        ...
        if (todaySessions.length > 0) {
            const averageRating = Math.round(todaySessions.reduce((sum, session) => sum + session.rating, 0) / todaySessions.length);
    ...
    async exportData(type) {
        if (type === "users") {
            ...
            const rows = users.map(user => `${user.id},${user.email},${user.subscriptionType},${user.isAdmin},${user.createdAt},${user.lastActive}`).join("\n");
            return headers + rows;
        }
            ...
            const rows = sessions.map(session => `${session.id},"${session.difficultyLevel}","${session.japaneseSentence.replace(/"/g, '""')}","${session.userTranslation.replace(/"/g, '""')}","${session.correctTranslation.replace(/"/g, '""')}",${session.rating},${session.createdAt.toISOString()}`).join("\n");
            return headers + rows;
        }
