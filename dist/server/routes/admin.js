...
import { userSubscriptions, trainingSessions } from "../../shared/schema.js";
...
export function registerAdminRoutes(app) {
    ...
    router.get("/users", async (req, res) => {
        try {
            ...
            const usersWithEmails = await Promise.all(subscriptions.map(async (subscription) => {
                try {
    ...
    router.get("/analytics", async (req, res) => {
        try {
            ...
            const categoryStatsGrouped = new Map();
    ...
    router.get("/export/:type", async (req, res) => {
        try {
            ...
            if (type === 'users') {
                ...
                const csvData = users.map(user => `${user.userId},${user.subscriptionType},${user.subscriptionStatus},${user.isAdmin},${user.createdAt},${user.updatedAt}`).join('\n');
                ...
                const csvData = sessions.map(session => `${session.id},${session.userId},${session.difficultyLevel},"${session.japaneseSentence}","${session.userTranslation}",${session.rating},${session.isBookmarked},${session.createdAt}`).join('\n');
                res.setHeader('Content-Type', 'text/csv');
