...
export class Storage {
    ...
    async getStreakCount(userId) {
        try {
            ...
            const recentSessions = sessions.filter(s => new Date(s.createdAt) >= yesterday);
    ...
    async getSessionsForReview(userId, threshold = "3") {
        try {
            const sessions = await this.getTrainingSessions(userId);
    ...
    async getRecentSessions(userId, daysBack = "7") {
        try {
            const sessions = await this.getTrainingSessions(userId);
