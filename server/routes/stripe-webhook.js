...
if (!stripeSecretKey || !webhookSecret) {
    ...
    // Stripeインスタンスを初期化
    var stripe_2 = new stripe_1.default(stripeSecretKey, {
        apiVersion: "2025-07-30.basil",
    });
    // Webhookルート
    router.post("/", express_1.default.raw({ type: "application/json" }), function (req, res) {
        var sig = req.headers["stripe-signature"];
        var event;
        try {
            if (!sig)
                throw new Error("Missing Stripe signature header");
            event = stripe_2.webhooks.constructEvent(req.body, sig, webhookSecret);
        }
        catch (err) {
        ...
        switch (event.type) {
            ...
            case "customer.subscription.deleted":
                ...
                break;
            default:
                console.log("\u26A0\uFE0F Unhandled event type: ".concat(event.type));
        }
