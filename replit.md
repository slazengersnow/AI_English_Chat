...
- **build.sh実行方式**: 最もシンプルで確実なビルド方式に統一
  - render.yamlでbuildCommand: ./build.sh
  - build.shでnpm ci --include=dev && npx vite build && npx esbuild...
...
- **トライアル期間表示**: スタンダードプランのトライアル中ユーザーに適切な表示を実装
  ...
  - 自動更新後のプランを現在のプランと同じに設定（スタンダード→スタンダード）
  - スタンダードプランの問題数制限を100問/日から50問/日に修正
- **認証システム修正**: JWTトークンからユーザーIDを正しく抽出
  - SupabaseのJWTトークンを適切に解析してユーザーメールアドレスを取得
  - 認証ミドルウェアでのユーザー識別を改善
  - スタンダードプランユーザーのプレミアム機能制限を正しく実装
...
- **AIプロバイダー切り替え**: OpenAI GPT-4oからAnthropic Claude 3 Haikuに移行完了
  ...
  - モデル: `claude-3-haiku-20240307`を使用（低コスト高性能）
  - API呼び出し方式: OpenAI Chat Completions → Anthropic Messages API
  - 応答速度の改善: 約3秒での高速応答（従来の7秒から60%向上）
...
- **プレミアム機能アクセス制限の修正**: スタンダードプランユーザーのプレミアム機能アクセス問題を解決
  - `useSubscription`フックの`canAccessPremiumFeatures`ロジックを修正
  - スタンダードプランユーザーはトライアル状態でもプレミアム機能（シミュレーション練習）にアクセス不可
  - プレミアムプランユーザーのみがプレミアム機能にフルアクセス可能
- **サーバーサイドAPI保護強化**: プレミアム認証ミドルウェアを実装
  - `requirePremiumSubscription`ミドルウェアを追加
  - シミュレーション関連APIエンドポイントを保護：`/api/simulation-problem/:scenarioId`、`/api/simulation-translate`
  - カスタムシナリオ管理APIエンドポイントを保護：`/api/custom-scenarios`（作成、更新、削除）
- **UI/UXメッセージの適切な分離**: 機能別にプレミアム案内メッセージを最適化
  - 繰り返し練習：「繰り返すだけで、フレーズが定着し、確実に話せる英語が増えていきます。」
  - シミュレーション練習：「リアルなビジネスシーンを想定したシミュレーション練習を体験したい方は、プレミアムプランをご検討ください。」
  - 各機能の価値提案を明確に区別したメッセージング
